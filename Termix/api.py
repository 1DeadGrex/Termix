...
app = FastAPI(title="ERVSE CS2 Tournament API", version="3.0.0", lifespan=lifespan)
...
# Helpers
# ─────────────────────────────────────────────
async def _dm_user(discord_id: int, content: str = None, embed: discord.Embed = None):
    """Best-effort DM a Discord user. Returns True on success."""
    if _bot is None:
        return False
    try:
        ...
        print(f"[dm] {discord_id} has DMs closed")
    except Exception as e:
        print(f"[dm] failed to DM {discord_id}: {e}")
    return False
...
async def _assign_role_to_discord_user(discord_id: int, role_id: int, reason: str = "Auto role"):
    ...
    for guild in _bot.guilds:
        ...
        try:
            ...
            print(f"[api] granted role {role.name} to {member}")
            return True
        except Exception as e:
...
@app.get("/")
async def root():
    return {"status": "ok", "service": "ERVSE CS2 Tournament API", "bot_ready": _bot is not None}
...
@app.get("/api/players/{user_id}/full")
async def get_player_full(user_id: int):
    """Full profile: player row + xp + record + last 10 matches."""
    row = await db.get_player(user_id)
    xp = await db.get_user_xp(user_id)
    wins, losses = await db.get_player_record(user_id)
    matches = await db.get_match_history(user_id, limit=10)
    ...
    # Resolve display names for the last 10 matches
    name_cache = {}
    async def name_of(uid):
        if uid in name_cache:
            return name_cache[uid]
        p = await db.get_player(uid)
        nm = (p or {}).get("discord_name") or f"OP-{str(uid)[-4:]}"
        name_cache[uid] = nm
        return nm

    match_list = []
    for m in matches:
        match_id, p1, p2, winner_id, score, m_name, m_map, played_at = m
        opponent_id = p2 if p1 == user_id else p1
        match_list.append({
            "id": match_id,
            "opponent_id": opponent_id,
            "opponent_name": await name_of(opponent_id),
            "winner_id": winner_id,
            "you_won": winner_id == user_id,
            "score": score,
            "match_name": m_name or "",
            "match_map": m_map or "",
            "played_at": played_at,
        })
...
# ─────────────────────────────────────────────
# Matches / leaderboard / bans / presence — unchanged
# ─────────────────────────────────────────────
@app.get("/api/matches")
...
async def update_tournament(tid: int, payload: TournamentPayload, request: Request):
    require_admin(request)
    if not await db.get_tournament(tid):
        raise HTTPException(status_code=404, detail="Tournament not found")
...
async def announce_tournament(tid: int, request: Request):
    ...
    try:
        ...
        if t.get("prize_pool") or t.get("prize_extra"):
            parts = []
            if t.get("prize_pool"): parts.append(f"${t['prize_pool']}")
            if t.get("prize_extra"): parts.append(t["prize_extra"])
        ...
        embed = discord.Embed(title=f"🎮 {t['name']}", description="\n".join(desc_lines), color=0xFFB000)
        if t.get("prize_image_url"):
            try: embed.set_image(url=t["prize_image_url"])
            except Exception: pass
        if t.get("rules_url"):
            embed.add_field(name="Rules", value=t["rules_url"], inline=False)
        embed.set_footer(text="Register with /register in Discord · ERVSE")
...
async def register_for_tournament(tid: int, payload: TournamentRegisterPayload):
    ...
    if player and player.get("steam_id"):
        ...
        if sb:
            raise HTTPException(status_code=403, detail=f"Steam account is banned: {sb.get('reason', 'no reason given')}")

    rid = await db.register_for_tournament(tid, payload.discord_id, payload.discord_username, payload.mode)
    if rid is None:
    ...
    if payload.status == "approved":
        ...
        if role_id:
            role_granted = await _assign_role_to_discord_user(
                reg["discord_id"], role_id, reason=f"Approved for tournament #{tid}"
            )
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"Your registration for **{t['name'] if t else 'the tournament'}** "
                    f"has been **approved**.\n\nHead to the Discord for check-in details."
                ),
                color=0x57E389,
            )
            await _dm_user(reg["discord_id"], embed=embed)
        except Exception:
            pass

    return {
        "status": "ok", "registration_id": rid,
        "new_status": payload.status, "role_granted": role_granted,
    }


# ─────────────────────────────────────────────
# Steam OpenID — with DM on success/fail
# ─────────────────────────────────────────────
@app.get("/auth/steam")
...
async def steam_callback(discord_id: int, request: Request):
    ...
    try:
        ...
        await _dm_user(discord_id, content="❌ **ERVSE Steam link failed** — couldn't reach Steam. Try again.")
        return _vhs_error("VERIFY FAILED", "Could not reach Steam. Try again.")

    if "is_valid:true" not in r.text:
        await _dm_user(discord_id, content="❌ **ERVSE Steam link failed** — Steam rejected the attempt. Try again.")
    ...
    if not steam_id64.isdigit():
        await _dm_user(discord_id, content="❌ **ERVSE Steam link failed** — no SteamID64 in response.")
        return _vhs_error("VERIFY FAILED", "No SteamID64 in Steam's response.")

    # Bans
    if await db.get_ban(discord_id):
        await _dm_user(discord_id, content="🚫 **ERVSE — link refused** — your Discord account is banned.")
        return _vhs_error("ACCESS DENIED", "Your Discord account is banned from ERVSE.")

    sb = await db.get_steam_ban(steam_id64)
    if sb:
        await _dm_user(
            discord_id,
            content=f"🚫 **ERVSE — link refused** — this Steam account is banned: `{sb.get('reason', 'no reason')}`",
        )
        return _vhs_error("ACCESS DENIED",
    ...
    if existing and existing["user_id"] != discord_id:
        await _dm_user(
            discord_id,
            content="🚫 **ERVSE — link refused** — this Steam account is already linked to another Discord user.",
        )
        return _vhs_error("ALREADY LINKED",
    ...
    await db.add_player(
        user_id=discord_id,
        discord_name="",              # don't overwrite existing discord_name
        steam_id=steam_id64,
        verified=True,
        steam_name=persona if persona != "Unknown" else f"Steam: {steam_id64}",
    )

    # DM confirmation
    try:
        embed = discord.Embed(
            title="✅ ERVSE — Steam Linked",
            description=(
                f"Your Steam account has been linked.\n\n"
                f"**Steam name:** `{persona}`\n"
        ...
        embed.set_footer(text="ERVSE · You're eligible for tournament entry")
        await _dm_user(discord_id, embed=embed)
    except Exception as e:
        print(f"[steam dm] {e}")
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
            r = await client.get(
                ...
                headers={"User-Agent": "Mozilla/5.0 (ERVSEBot/1.0)"},
            )
...
<title>CLEARANCE GRANTED — ERVSE</title>
...
.btn{{display:inline-flex;align-items:center;justify-content:center;gap:10px;font:16px/1.25 var(--mono);letter-spacing:2px;padding:15px 20px;cursor:pointer;border:2px solid;text-decoration:none;text-align:center}}
...
<div class="wrap">
  <div class="panel">
    ...
    <div class="body">
      ...
      <div class="dossier">
        <div class="frame"><img src="{esc_avatar or default_avatar_svg}" alt="Steam avatar"></div>
        <div class="idrows">
...
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{html_lib.escape(title)} — ERVSE</title>
<link href="https://fonts.googleapis.com/css2?family=Press+Start+2P&family=VT323&display=swap" rel="stylesheet">
<style>
